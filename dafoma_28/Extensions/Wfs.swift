...
class WController: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    func loadCookie() {
        ...
        if let data = ud.object(forKey: "cookie") as? Data, let cookies = try? NSKeyedUnarchiver.unarchivedObject(ofClass: NSArray.self, from: data) as? [HTTPCookie] {
